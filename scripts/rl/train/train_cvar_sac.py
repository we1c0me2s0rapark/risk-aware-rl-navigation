...
def preprocess_obs(obs, config, device) -> dict | None:
    """
    @brief Preprocess raw CARLA environment observations for CVaR-SAC input.
...
def main():
    """
    @brief Main CVaR-SAC training loop for CARLA environment.

    @details
    Structurally identical to train_sac.py. Key differences:
        - Uses CVaRSACAgent instead of SACAgent
        - Critic outputs 32 quantiles per forward pass
        - Actor is optimised using CVaR_0.1(Z) instead of E[Q]
        - Logs additional cvar/mean metric to TensorBoard

    The distributional critic learns the full return distribution Z(s,a),
    and CVaR focuses the policy on reducing worst-case outcomes — collisions
    and near-miss events rather than just maximising expected reward.
    """
    fd = sys.stdin.fileno()
    ...
    try:
        ...
        log_dir = os.path.abspath(os.path.join(
            ws_root_path, render_root, "runs", "cvar_sac"
        ))
        logger = TrainingLogger(log_dir=log_dir, algorithm="cvar_sac")
        ...
        Log.info(__file__, f"Starting CVaR-SAC training on {device}")
        ...
        while step_count < total_steps:
            ...
            # Normalise next_obs before storing
            ...
            if step_count >= learning_starts:
                ...
                # Log CVaR alpha reference point periodically
                if step_count % log_every == 0:
                    logger.log_cvar_alpha(step_count, cvar_alpha)
